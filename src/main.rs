...
fn main() -> io::Result<()> {
    ...
    for i in 1..args.len() {
        ...
        if flag == "-v" || flag == "--version" {
            println!("smosummary v2.4.0-1 | owner: aray4iv3");
            return Ok(());
    ...
    if choice == 1 {
        active = vec!["cap", "cascade", "sand", "lake", "wooded", "cloud", "lost", "metro", "snow", "seaside", "luncheon", "ruined", "bowser", "moon"].into_iter().map(String::from).collect();
    } else if choice == 2 {
        active = vec!["mushroom", "cap", "cascade", "sand", "lake", "wooded", "cloud", "lost", "metro", "snow", "seaside", "luncheon", "ruined", "bowser", "moon", "dark", "darker"].into_iter().map(String::from).collect();
    } else if choice == 3 {
        ...
        loop {
            ...
            if trimmed == "done" || trimmed.is_empty() { break; }
    ...
    if stopwatch {
        ...
        for k in &active {
            ...
            let mut f = OpenOptions::new().create(true).append(true).open(&save_path)?;
